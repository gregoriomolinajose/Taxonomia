...
1. **Definición Maestra del Esquema (APP_SCHEMAS):** Definir la nueva entidad en el motor de esquemas. El esquema debe ser un objeto polimórfico (Diccionario de Objetos) y declarar explícitamente el `primaryKey` y `titleField`. Todo nuevo esquema DEBE acatar las siguientes 5 directrices de UI/Datos:
   ...
   * **Protocolo 1:N (`dynamic_list`):** Prohibido crear hojas de Google Sheets "hijas" para colecciones pequeñas de metadatos. Usar `type: "dynamic_list"` con un arreglo de `subFields`. El frontend lo renderizará como sub-grilla y el backend lo almacenará en una sola celda como JSON stringificado.
