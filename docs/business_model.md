...
## 8. Blueprint Estricto para Nuevos Nodos MDM V4 (SAFe 6.0 & SCD-2 Graph)
Cuando el usuario ordene crear una "Nueva Entidad" o un "Nuevo Módulo", el Agente tiene ESTRICTAMENTE PROHIBIDO improvisar código en el HTML. DEBE ejecutar esta lista de verificación exacta, garantizando que el formulario se renderice como un Wizard interactivo y mantenga la integridad relacional del grafo:

1. **Definición Maestra del Esquema (APP_SCHEMAS):** Definir la nueva entidad en el motor de esquemas. El esquema debe ser un objeto polimórfico (Diccionario de Objetos) y declarar explícitamente el `primaryKey` y `titleField`. Todo nuevo esquema DEBE acatar las siguientes 7 directrices de UI/Datos:
   ...
   * **Integridad Foránea e In-Line Modal Stack (`lookupSource`):** Prohibido pedir nombres manualmente si existe la entidad. Usar `type: "select"` con `lookupSource: "getEntidadOptions"`. Esto habilita automáticamente el **Modal Stack LIFO**, permitiendo al usuario crear el registro padre/hijo en caliente sin salir del formulario.
   * **Topología Estructural (ADR-002):** Si la entidad participa del Grafo Relacional Temporal, OBLIGATORIAMENTE declarar `isTemporalGraph: true` y asentar su cardinalidad (`topology: "JERARQUICA_LINEAL"`, etc.) para invocar los cierres de fechas `valido_hasta` automáticos en el Backend.
   * **Renderizado de Relaciones 1:N (One-to-Many):** Prohibido dejar a la improvisación. Si la entidad posee colecciones atadas fuertes en BD separada (ej. Tareas de un Proyecto), el esquema delega su carga visual EXCLUSIVAMENTE al componente **`UI_SubgridBuilder`**. Si son colecciones débiles y pequeñas sin su propia hoja de cálculo (BD), usar `type: "dynamic_list"` para renderizar una grilla in-line que el backend salva como JSON stringificado.
   * **Renderizado de Relaciones M:N (Many-to-Many):** Las lógicas puente, donde múltiples identidades cruzan con otras múltiples (Ej. Capacidades vinculadas a muchos Productos), están forzadas a utilizar el componente **`chip-input`** (para arrays de IDs nativos) o delegar al **`UI_SubgridBuilder`** si requieren campos asociativos adicionales. Esto blinda matemáticamente el cruce de UUIDs impidiendo la corrupción de la normalización.
   * **Minimalismo en DataGrid (`showInList`):** Prohibido saturar las tablas principales (DataView) con campos de texto extensos como "Comentarios", "Descripciones largas" o "Justificaciones". La cuadrícula debe conservar su esencia de mostrar exclusivamente la información mínima vital. Estos campos densos deben ocultarse de la vista de lista (ej. `showInList: false`), forzando al usuario a abrir el registro completo (Drill-down) para leer el detalle.

2. **Registro de Metadatos (Single Source of Truth):** Registrar OBLIGATORIAMENTE la entidad en `window.ENTITY_META` (o archivo global correspondiente), mapeando su `idField`, `titleField`, `iconName`, `color` y su índice de `order`. **Prevención de Saturación Visual:** NO toda entidad debe inyectarse indiscriminadamente (Ej. un catálogo menor o tabla puente). Se debe declarar explícitamente mediante *flags* booleanos en la metadata (ej. `showInMenu: true`, `showInDashboard: false`) si la entidad amerita acceso principal, evitar el colapso cognitivo en el UI.

3. **Auto-Aprovisionamiento Dinámico (Backend):** Garantizar que, si la base de datos (Google Sheets) no tiene la pestaña física, el Backend la cree extrayendo dinámicamente las llaves del esquema polimórfico, inyectando de forma inquebrantable el ID Maestro declarado en **`primaryKey`** y sumando obligatoriamente las **6 columnas de auditoría** (`created_at`, `created_by`, `updated_at`, `updated_by`, `deleted_at`, `deleted_by`).

4. **Sanitización Doble (Falsy Trap & MALS Rule):**
   * **En el Frontend:** Antes de empaquetar, OBLIGATORIO hacer `String(val).trim()` a los valores para evitar la propagación de nulos corruptos (`""`) que pasen las barreras como *Truthiness* hacia un Backend OCP.
   * **En el Backend:** Aplicar `JSON.parse(JSON.stringify(record))` para destruir objetos `Date` nativos antes de cruzar la red hacia el frontend previniendo colapsos de de-serialización.

5. **Soft-Delete Integrado:** Toda entidad deberá someterse a la regla estricta de Soft-Delete (`estado !== 'Eliminado'`) en el motor de frontend, previniendo borrados físicos que destruyan las líneas históricas de auditoría en la Base de Datos.

6. **Cero-Toque en UI (Zero-Touch HTML):** Queda ESTRICTAMENTE PROHIBIDO modificar manualmente los archivos `Index.html` o `DataView_UI.html` para agregar pestañas, tarjetas o ítems de menú. La UI es 100% reactiva interpolando el esquema.

7. **Build Tracking (Regla de Invalidación):** Incrementar obligatoriamente la versión de compilación (`APP_VERSION` o Build Tracker) en cada despliegue para forzar la invalidación del caché visual del navegador en el cliente.

8. **Motor de Carga/Descarga Universal (Bulk Ops):** La importación debe acoplarse estáticamente a `bulkInsert`, forzando el procesamiento matricial (`setValues()`) de lotes sin iterar interacciones I/O en bucles secuenciales.