...
2. **Registro de Metadatos (Single Source of Truth):** Registrar OBLIGATORIAMENTE la entidad en `window.ENTITY_META` (o archivo global correspondiente), mapeando su `idField`, `titleField`, `iconName`, `color` y su índice de `order`. Esto alimentará automáticamente el menú lateral y el dashboard inmutablemente.
