...
- **Auditoría de Rehidratación de Subgrids:** El Tester (manual o E2E) tiene la obligación de verificar no solo la tabla principal tras una adición o actualización, sino que DEBE cerrar y volver a abrir la Entidad Padre (Top-Level Element) para verificar sin recarga (0ms Cache Hit) que los registros anidados (Subgrids) aparezcan correctamente listados localmente sin recurrir al servidor (Relational Local Refreshing).