...
- Name: dashboard-counters.spec.js >> E28: Dashboard Counters Visibility >> Los contadores del dashboard deben renderizar datos númericos en lugar de spinners (... o vacíos)
...
Call log:
  - waiting for locator('#sandboxFrame').contentFrame().locator('#userHtmlFrame').contentFrame().locator('ion-item').filter({ hasText: 'Inicio' })
...
```ts
  ...
  34 |     await page.goto('https://script.google.com/macros/s/AKfycbz1wK6yVfTuUe3kv35k45IULLBVya51t6HDXUZHNP-6rI9Nh_PEctWZseGyoQiQ2HxkIw/exec');
  35 |     
...
> 45 |     await frame.locator('ion-item').filter({ hasText: 'Inicio' }).click();
  ...
  47 |   });
  48 | 
  49 |   test('Los contadores del dashboard deben renderizar datos númericos en lugar de spinners (... o vacíos)', async () => {
  ...
  76 |   });
  77 | 
  78 |   test('La invocación a getDashboardCounters no debe emitir el error deserialize threw error', async () => {
  ...
  87 |       await page.goto('https://script.google.com/macros/s/AKfycbz1wK6yVfTuUe3kv35k45IULLBVya51t6HDXUZHNP-6rI9Nh_PEctWZseGyoQiQ2HxkIw/exec');
