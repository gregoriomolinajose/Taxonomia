window.UI_DataView_Toolbar = (function () {
    ...
    ───────────────────────────────────────────── */
    function buildHeader(entityName, totalRecords, canCreate, onExportCSV, onImportCSVTrigger, onAddClick, onBulkDeleteClick) {
        ...
        btnBulkDelete.id = 'dv-bulk-delete-btn';
        btnBulkDelete.className = 'dv-btn dv-btn-danger';
        btnBulkDelete.style.display = 'none'; // Oculto por defecto
        if (typeof onBulkDeleteClick === 'function') btnBulkDelete.addEventListener('click', onBulkDeleteClick);
        const iconDel = document.createElement('ion-icon');
        iconDel.setAttribute('name', 'trash-outline');
        iconDel.setAttribute('slot', 'start');
        btnBulkDelete.appendChild(iconDel);
        btnBulkDelete.appendChild(document.createTextNode(' Eliminar Seleccionados'));
        
        rightDiv.appendChild(btnBulkDelete);
