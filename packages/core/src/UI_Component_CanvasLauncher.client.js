...
window.UI_Component_CanvasLauncher = {
    build: function(field, container, data, entityName, localEventBus, modalContext, config = {}) {
        ...
        function openCanvasModal(id, sourceEntity, passedExplicitContext) {
            if (sourceEntity === 'Flujo') {
                if (typeof window.UI_View_FlujoCanvas !== 'undefined') {
                    window.UI_View_FlujoCanvas.buildModal(id, passedExplicitContext);
                } else {
                    console.warn('UI_View_FlujoCanvas no está cargado');
