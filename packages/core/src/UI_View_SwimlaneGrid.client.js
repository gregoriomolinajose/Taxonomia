...
window.UI_View_SwimlaneGrid = {
    ...
    _buildCanvas: function(rootContainer) {
        ...
        const edgeExtractors = [
            ...
            { component: 'flujos_vinculados', parentField: 'id_portafolio', edgeType: 'PORTAFOLIO_FLUJO' },
            { component: 'grupos_productos_vinculados', parentField: 'id_flujo', edgeType: 'FLUJO_GRUPO_PRODUCTO' },
            { component: 'dominios_vinculados', parentField: 'id_flujo', edgeType: 'FLUJO_DOMINIO' },
                        ...
                        arr.forEach(childItem => {
                            if (childItem) {
                                ...
                                if (typeof childItem === 'object') {
                                    validChildId = childItem.id_registro || childItem.id_portafolio || childItem.id_flujo || childItem.id_grupo_producto || childItem.id_dominio || childItem.id_equipo || Object.values(childItem)[0];
        ...
        if (portafolioEdges.length > 0) {
            ...
            portafolioEdges.forEach(pEdge => {
                ...
                const valueStreamEdges = contextEdges.filter(e => 
                    e.tipo_relacion === 'PORTAFOLIO_FLUJO' && 
                ...
                if (valueStreamEdges.length > 0) {
                    ...
                    valueStreamEdges.forEach(vsEdge => {
                        ...
                        vsCol.appendChild(this._createNodeEl(vsId, 'Flujo', 'Editar Value Stream'));
                        ...
                        const grupoEdges = contextEdges.filter(e => 
                            e.tipo_relacion === 'FLUJO_GRUPO_PRODUCTO' && 
                        ...
                        if (grupoEdges.length === 0 && dominioEdges.length === 0) {
                            ...
                            btnAddGP.onclick = (e) => {
                                e.stopPropagation();
                                this._handleNodeAdd(vsId, 'Flujo', e);
                            };
                    ...
                    // Empty State Onboarding para Flujos
...
_initPanZoom: function(viewport, canvas) {
    ...
    _createNodeEl: function(recordId, entityName, addTitle, edgeData) {
        ...
        if (window.Graph_Utils && typeof window.Graph_Utils.resolveAllLinkedIds === 'function') {
            ...
            if (entityName === 'Unidad_Negocio') {
                ...
                edgeType = 'PORTAFOLIO_FLUJO'; childLabelSingle = 'Flujo'; childLabelPlural = 'Flujos';
            } else if (entityName === 'Flujo') {
                const gpIds = window.Graph_Utils.resolveAllLinkedIds(recordId, 'FLUJO_GRUPO_PRODUCTO', this.taxonomiaId, false, 'hijo');
                const domIds = window.Graph_Utils.resolveAllLinkedIds(recordId, 'FLUJO_DOMINIO', this.taxonomiaId, false, 'hijo');
    ...
    _handleNodeAdd: function(parentId, parentEntity, ev) {
        ...
        if (parentEntity === 'Root_Taxonomia') {
            ...
            childEntity = 'Flujo';
            edgeType = 'PORTAFOLIO_FLUJO';
        } else if (parentEntity === 'Flujo') {
            childEntity = 'Grupo_Productos';
            edgeType = 'FLUJO_GRUPO_PRODUCTO';
        ...
        if (recordData && typeof window.renderForm === 'function') {
            ...
            if (edgeType === 'PORTAFOLIO_FLUJO') targetSection = 'Flujos Vinculados';
            if (edgeType === 'FLUJO_GRUPO_PRODUCTO') targetSection = 'Grupos de Productos';
            if (edgeType === 'FLUJO_DOMINIO') targetSection = 'Dominios Vinculados';
